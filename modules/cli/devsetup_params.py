...
RC_PARAMS_FILE = f"{Path.home()}/.zshrc.d/user_env.sh"
...
def read_parameters():
    results = client.get_parameters_by_path(Path=PARAM_KEY_PREFIX, Recursive=True)
    parameters = []
    for param in results["Parameters"]:
        ...
        for t in tags:
            if t["Key"] == "OriginalName":
                ...
                break
        parameters.append(
            {"Key": param["Name"], "Name": original_name, "Value": param["Value"]}
        )
    return parameters
...
def put_parameter(name, value):
    ...
    options = {}
    try:
        existing = client.get_parameter(Name=key)
        options["Overwrite"] = True
    except client.exceptions.ParameterNotFound:
        options["Tags"] = [{"Key": "OriginalName", "Value": name}]

    try:
        return client.put_parameter(Name=key, Value=value, Type="String", **options)
...
def put():
    ...
    if status_code == 200:
        ...
        raise ValueError(f"Error while storing the SSM parameter: {status_code}")
